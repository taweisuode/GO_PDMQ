...
// Producer is a high-level type to publish to PDMQ.
//
// A Producer instance is 1:1 with a destination `pdmqd`
...
// Ping causes the Producer to connect to it's configured pdmqd (if not already
...
// but does not wait for the response from `pdmqd`.
//
// When the Producer eventually receives the response from `pdmqd`,
...
func (w *Producer) connect() error {
	...
	w.log(LogLevelInfo, "(%s) connecting to pdmqd", w.addr)
	...
	if err != nil {
		w.conn.Close()
		w.log(LogLevelError, "(%s) error connecting to pdmqd - %s", w.addr, err)
		return err
	}
