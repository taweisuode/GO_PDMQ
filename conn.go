...
// returned from an IDENTIFY command to pdmqd
...
// returned from an AUTH command to pdmqd
...
// Conn represents a connection to pdmqd
//
...
// MaxRDY returns the pdmqd negotiated maximum
...
// RemoteAddr returns the configured destination pdmqd address
...
func (c *Conn) writeLoop() {
	for {
		...
		case resp := <-c.msgResponseChan:
			// Decrement this here so it is correct even if we can't respond to pdmqd
...
func (c *Conn) close() {
	...
	//
	//     1. CLOSE cmd sent to pdmqd
	//     2. CLOSE_WAIT response received from pdmqd
	//     3. set c.closeFlag
	//     4. readLoop() exits
