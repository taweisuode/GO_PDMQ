...
func newMockPDMQD(t *testing.T, script []instruction, addr string) *mockPDMQD {
	n := &mockPDMQD{
		t:        t,
		script:   script,
...
func (n *mockPDMQD) handle(conn net.Conn) {
	var idx int
...
func TestConsumerBackoff(t *testing.T) {
	...
	n := newMockPDMQD(t, script, addr.String())
	...
	err := q.ConnectToPDMQD(n.tcpAddr.String())
	if err != nil {
		t.Fatalf(err.Error())
	}
...
func TestConsumerBackoffDisconnect(t *testing.T) {
	...
	n = newMockPDMQD(t, script, n.tcpAddr.String())
