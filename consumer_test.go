...
func consumerTest(t *testing.T, cb func(c *Config)) {
	...
	err = q.ConnectToPDMQD(addr)
	if err == nil {
		t.Fatal("should not be able to connect to the same PDMQ twice")
	}
	...
	err = q.DisconnectFromPDMQD("1.2.3.4:4150")
	if err == nil {
		t.Fatal("should not be able to disconnect from an unknown pdmqd")
	}

	err = q.ConnectToPDMQD("1.2.3.4:4150")
	if err == nil {
		t.Fatal("should not be able to connect to non-existent pdmqd")
	}

	err = q.DisconnectFromPDMQD("1.2.3.4:4150")
	if err != nil {
		t.Fatal("should be able to disconnect from an pdmqd - " + err.Error())
	}
