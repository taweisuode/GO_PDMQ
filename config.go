...
// Config is a struct of PDMQ options
//
...
type Config struct {
	...
	// LocalAddr is the local address to use when dialing an pdmqd.
	...
	//
	// NOTE: when not using pdmqlookupd, LookupdPollInterval represents the duration of time between
	...
	// Duration to wait for a message from an pdmqd when in a state where RDY
	// counts are re-distributed (e.g. max_in_flight < num_producers)
	LowRdyIdleTimeout time.Duration `opt:"low_rdy_idle_timeout" min:"1s" max:"5m" default:"10s"`
	// Duration to wait until redistributing RDY for an pdmqd regardless of LowRdyIdleTimeout
	...
	// Identifiers sent to pdmqd representing this client
	...
	// Integer percentage to sample the channel (requires pdmqd 0.2.25+)
	...
	// Size of the buffer (in bytes) used by pdmqd for buffering writes to this connection
	OutputBufferSize int64 `opt:"output_buffer_size" default:"16384"`
	// Timeout used by pdmqd before flushing buffered writes (set to 0 to disable).
	//
	// WARNING: configuring clients with an extremely low
	// (< 25ms) output_buffer_timeout has a significant effect
	// on pdmqd CPU usage (particularly with > 50 clients connected).
	...
	// secret for pdmqd authentication (requires pdmqd 0.2.29+)
	AuthSecret string `opt:"auth_secret"`
}

// NewConfig returns a new default pdmq configuration.
//
...
func (h *structTagsConfig) Set(c *Config, option string, value interface{}) error {
	...
	for i := 0; i < typ.NumField(); i++ {
		...
		if coercedVal.Type().String() == "pdmq.BackoffStrategy" {
...
func (h *structTagsConfig) SetDefaults(c *Config) error {
	...
	c.Hostname = hostname
	c.UserAgent = fmt.Sprintf("go-pdmq/%s", VERSION)
	return nil
}
