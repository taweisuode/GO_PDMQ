...
//
// 	consumer.AddHandler(pdmq.HandlerFunc(func(m *Message) error {
// 		// handle the message
// 	}))
...
// for filtering the pdmqds returned from discovery via pdmqlookupd
...
// Consumer is a high-level type to consume from PDMQ.
//
...
//
// If configured, it will poll pdmqlookupd instances and handle connection (and
// reconnection) to any discovered pdmqds.
...
//
// This may change dynamically based on the number of connections to pdmqd the Consumer
// is responsible for.
...
// ConnectToPDMQLookupd adds an pdmqlookupd address to the list for this Consumer instance.
//
// If it is the first to be added, it initiates an HTTP request to discover pdmqd
...
// ConnectToPDMQLookupds adds multiple pdmqlookupd address to the list for this Consumer instance.
//
// If adding the first address it initiates an HTTP request to discover pdmqd
// producers for the configured topic.
//
// A goroutine is spawned to handle continual polling.
...
// make an HTTP req to one of the configured pdmqlookupd instances to discover
...
retry:
	...
	if err != nil {
		...
		retries++
		if retries < 3 {
			r.log(LogLevelInfo, "retrying with next pdmqlookupd")
			goto retry
		}
		return
	}

	var pdmqdAddrs []string
	for _, producer := range data.Producers {
		broadcastAddress := producer.BroadcastAddress
		port := producer.TCPPort
		joined := net.JoinHostPort(broadcastAddress, strconv.Itoa(port))
		pdmqdAddrs = append(pdmqdAddrs, joined)
	}
	// apply filter
	...
	for _, addr := range pdmqdAddrs {
		...
		if err != nil && err != ErrAlreadyConnected {
			r.log(LogLevelError, "(%s) error connecting to pdmqd - %s", addr, err)
			continue
		}
	}
}

// ConnectToPDMQDs takes multiple pdmqd addresses to connect directly to.
//
// It is recommended to use ConnectToPDMQLookupd so that topics are discovered
// automatically.  This method is useful when you want to connect to local instance.
func (r *Consumer) ConnectToPDMQDs(addresses []string) error {
	for _, addr := range addresses {
		err := r.ConnectToPDMQD(addr)
		if err != nil {
			return err
...
func (r *Consumer) ConnectToPDMQD(addr string) error {
	if atomic.LoadInt32(&r.stopFlag) == 1 {
		return errors.New("consumer stopped")
	}
	...
	if idx := indexOf(addr, r.pdmqdTCPAddrs); idx == -1 {
		r.pdmqdTCPAddrs = append(r.pdmqdTCPAddrs, addr)
	}
	r.mtx.Unlock()

	r.log(LogLevelInfo, "(%s) connecting to pdmqd", addr)
...
// DisconnectFromPDMQD closes the connection to and removes the specified
// `pdmqd` address from the list
func (r *Consumer) DisconnectFromPDMQD(addr string) error {
	r.mtx.Lock()
	defer r.mtx.Unlock()

	idx := indexOf(addr, r.pdmqdTCPAddrs)
	if idx == -1 {
		return ErrNotConnected
	}

	// slice delete
	r.pdmqdTCPAddrs = append(r.pdmqdTCPAddrs[:idx], r.pdmqdTCPAddrs[idx+1:]...)
...
// DisconnectFromPDMQLookupd removes the specified `pdmqlookupd` address
// from the list used for periodic discovery.
func (r *Consumer) DisconnectFromPDMQLookupd(addr string) error {
	r.mtx.Lock()
	defer r.mtx.Unlock()
	...
	if len(r.lookupdHTTPAddrs) == 1 {
		return fmt.Errorf("cannot disconnect from only remaining pdmqlookupd HTTP address %s", addr)
	}
...
func (r *Consumer) onConnResponse(c *Conn, data []byte) {
	...
	case bytes.Equal(data, []byte("CLOSE_WAIT")):
		...
		r.log(LogLevelInfo, "(%s) received CLOSE_WAIT from pdmqd", c.String())
		c.Close()
	}
}
...
func (r *Consumer) onConnClose(c *Conn) {
	...
	r.mtx.RLock()
	numLookupd := len(r.lookupdHTTPAddrs)
	reconnect := indexOf(c.String(), r.pdmqdTCPAddrs) >= 0
	r.mtx.RUnlock()
	if numLookupd > 0 {
		...
		// there are no lookupd and we still have this pdmqd TCP address in our list...
		...
		go func(addr string) {
			for {
				...
				r.mtx.RLock()
				reconnect := indexOf(addr, r.pdmqdTCPAddrs) >= 0
				r.mtx.RUnlock()
				if !reconnect {
...
func (r *Consumer) updateRDY(c *Conn, count int64) error {
	...
	// never exceed the pdmqd's configured max RDY count
	if count > c.MaxRDY() {
		count = c.MaxRDY()
	}
...
//
// This panics if called after connecting to PDMQD or PDMQ Lookupd
//
